...
layout = [
    ...
    [
        ...
        sg.Combo(['no',"32px","48px_ctc"], readonly=True, key="ocr", default_value="no"),
        sg.Text('upscale-ratio放大', background_color="#e3f2fd", text_color='#212121'),
        sg.Combo(['no',1,2,3,4,8,16,32], readonly=True, key="ratio", default_value="no")
    ],
    [
...
def startocr(obj):
    ...
    cmd = "{pybin}  -i {sourcedir} -o {targetdir}  -f {imgformat} --translator={lang} --detector {detector} {heibai} {ocr}  {ratio}".format(pybin=pybin,  sourcedir=obj['sourcedir'], targetdir=obj['targetdir'], lang=obj['lang'], imgformat=obj['imgformat'], detector=obj['detector'], heibai=obj['heibai'],ocr=obj['ocr'],ratio=obj['ratio'])
    print(cmd)
    if obj['usegpu']:
        cmd += " --use-cuda"
    ...
    # 选择目标图片文件夹
        ...
        if current_status == 'ing':
            ...
            if obj['ratio']=='no':
                ...
                obj['ratio']=' --upscale-ratio %s'%obj['ratio']
